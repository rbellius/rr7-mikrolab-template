...
function checkCrds {
    ...
    # while [ true ]; do
        ...
        # "HelmChart" "Application" "ApplicationSet" "ResourceGraphDefinition" "CertManagerBundle" "MikroLab" "Argofile" "IngressRequest" "WebApp"; do
        local filter=""
        for kind in "$@"; do
            ...
            filter="$filter .spec.names.kind == \"${kind}\"";

        done
        local jqSelect=".items[] | select( ${filter} ) | { kind: .spec.names.kind, established: .status.conditions[] | select ( .type == \"Established\" ) | .status, namesAccpeted: .status.conditions[] | select( .type == \"NamesAccepted\" ) | .status, icon: \"Progressing\" }"
        echo $jqSelect

        local status=$(  kubectl get --kubeconfig mikrolab/kubeconfig.yaml crds -o json | jq -a "${jqSelect}");
