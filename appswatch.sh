...
function checkCrds {
    ...
    # while [ true ]; do
        ...
        local message=$(echo -e $status | jq -a '"\(.icon) Kind: \(.kind) \t| Established: \(.established) \t| Names Accepted: \(.namesAccpeted)"' | sed 's/"//g' );
        echo -e "$message\n" 
